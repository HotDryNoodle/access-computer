...
double json_get_number(const nlohmann::json& obj,
                       const char*           key,
                       double                fallback) {
...
std::string load_template(const std::filesystem::path& template_path) {
    ...
    if (!in) {
        throw std::runtime_error("Failed to open GMAT template: " +
...
std::filesystem::path default_gmat_root() {
    ...
    throw std::runtime_error(
        "GMAT install root not configured: set GMAT_ROOT or "
        "request.gmat.install_root");
}
...
GmatPaths resolve_gmat_paths(const nlohmann::json& request) {
    ...
    if (paths.console_binary.empty()) {
        paths.console_binary =
            paths.install_root / "bin" / "GmatConsole-R2026a";
    }
    return paths;
}
...
std::string render_optical_access_script(
    ...
    const auto& constraints = request.contains("constraints")
...
std::string render_downlink_script(const nlohmann::json&        request,
    ...
    script = replace_all(script, "{{STATION_LON}}",
                         std::to_string(target.at("lon_deg").get<double>()));
    script = replace_all(script, "{{STATION_ALT_KM}}",
                         std::to_string(target.at("alt_km").get<double>()));
    script =
        replace_all(script, "{{CONE_ANGLE_DEG}}", std::to_string(cone_deg));
    script = replace_all(script, "{{MIN_ELEVATION_DEG}}",
                         std::to_string(min_elevation_deg));
    return script;
}
...
GmatRunResult run_gmat_console(const GmatPaths&             paths,
    ...
    if (!std::filesystem::exists(paths.console_binary)) {
        throw std::runtime_error("GMAT console binary not found: " +
                                 paths.console_binary.string());
    }
    ...
    cmd << "LD_LIBRARY_PATH="
        << shell_quote(bin_dir.string() + ":" + plugins_dir.string())
        << ":$LD_LIBRARY_PATH ";
    cmd << shell_quote(paths.console_binary) << " --run "
        << shell_quote(abs_script_path);
