...
void print_global_usage() {
    std::cerr
        << "access-computer " << kVersion
        << " - agent-friendly access windows compute CLI plugin\n\n"
        << "Usage:\n"
        << "  access-computer manifest [--output json|json-pretty]\n"
        << "  access-computer validate (--input FILE | --stdin) [--output json|json-pretty]\n"
        << "  access-computer run (--input FILE | --stdin) --work-dir DIR [--dry-run] [--output json|json-pretty]\n\n"
        << "Global options:\n"
        << "  --help, -h       Show help (global or per subcommand)\n"
        << "  --version        Print version and exit\n\n"
        << "Examples:\n"
        << "  access-computer manifest --output json\n"
        << "  access-computer validate --input samples/remote_sensing_access.json\n"
        << "  access-computer run --input samples/remote_sensing_access.json --work-dir /tmp/mp_task\n"
        << "  cat samples/remote_sensing_access.json | access-computer run --stdin --work-dir /tmp/mp_task --dry-run\n";
}

void print_manifest_usage() {
    std::cerr
        << "access-computer manifest - print plugin manifest JSON\n\n"
        << "Usage:\n"
        << "  access-computer manifest [--output json|json-pretty]\n\n"
        << "Examples:\n"
        << "  access-computer manifest --output json\n"
...
void print_validate_usage() {
    std::cerr
        << "access-computer validate - validate a planning request without running GMAT\n\n"
        << "Usage:\n"
        << "  access-computer validate --input request.json [--output json|json-pretty]\n"
        << "  access-computer validate --stdin [--output json|json-pretty]\n\n"
        << "Examples:\n"
        << "  access-computer validate --input samples/remote_sensing_access.json\n"
        << "  access-computer validate --input samples/sar_unsupported.json\n"
        << "  cat samples/attitude_estimation.json | access-computer validate --stdin\n";
}

void print_run_usage() {
    std::cerr
        << "access-computer run - execute access windows compute (GMAT backend)\n\n"
        << "Usage:\n"
        << "  access-computer run --input request.json --work-dir DIR [--dry-run] [--output json|json-pretty]\n"
        << "  access-computer run --stdin --work-dir DIR [--dry-run] [--output json|json-pretty]\n\n"
        << "Options:\n"
        ...
        << "Examples:\n"
        << "  access-computer run --input samples/remote_sensing_access.json --work-dir /tmp/mp_task\n"
        << "  access-computer run --input samples/remote_sensing_access.json --work-dir /tmp/mp_task --dry-run\n"
        << "  cat samples/downlink_window.json | access-computer run --stdin --work-dir /tmp/mp_downlink\n";
}
    ...
    try {
        ...
        if (opts.command == "validate") {
            if (!opts.input_path && !opts.use_stdin) {
                ...
                std::cerr << "  access-computer validate --input samples/remote_sensing_access.json\n";
                std::cerr << "  cat request.json | access-computer validate --stdin\n";
        ...
        if (opts.command == "run") {
            if (!opts.work_dir) {
                ...
                std::cerr << "  access-computer run --input request.json --work-dir /tmp/task_xxx\n";
                std::cerr << "  access-computer run --stdin --work-dir /tmp/task_xxx --dry-run\n";
                return satellite::EXIT_VALIDATION;
            }
            if (!opts.input_path && !opts.use_stdin) {
                std::cerr << "Error: --input request.json or --stdin is required for run\n";
                std::cerr << "  access-computer run --input request.json --work-dir /tmp/task_xxx\n";
                std::cerr << "  cat request.json | access-computer run --stdin --work-dir /tmp/task_xxx\n";
                return satellite::EXIT_VALIDATION;
            }
            const auto request = satellite::read_json_input(opts.input_path, opts.use_stdin);
            mp::RunContext ctx;
